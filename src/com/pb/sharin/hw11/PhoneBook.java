...
public class PhoneBook {
    ...
    static void startMenu() {
        ...
        System.out.println("\t1. Поиск контактов по адресу - нажмите \"4\".");
    ...
    static void exportContacts(ArrayList<Contact> contactsExport) throws JsonProcessingException {
        ...
        Path pathWrite = Paths.get("files/export.json");
    ...
    public static void main(String[] args) throws JsonProcessingException {
        ...
        System.out.println("hw11 \"Collections\"");
        //==============================================================================
        ...
        while (provMenu == 0) {
            ...
            if (provChoice == 2) {
                ...
                for (int i = 0; i < contacts.size(); i++) {
                    boolean res = delFio.equals(contacts.get(i).getFio());
                    System.out.println(i);
                    if (res == true) {
                        qtyElem++;
                        Contact del = contacts.remove(i);
                    }
                }
                //Проверяем изменилось ли значение, найден ли индекс
                if (qtyElem == 0) {
                    System.out.println("-------------------------------------");
                    System.out.println("Контакты для удаления не найдены.");
                } else {
                    System.out.println("-------------------------------------");
                    System.out.println("Удалено " + qtyElem + " контакт(ов).");
                }
                //==================================================================
                */
                } catch (Exception ex) {
            ...
            if (provChoice == 4) {
                try {
                    ...
                    for (int i = 0; i < contacts.size(); i++) {
                        boolean res = provFio.equals(contacts.get(i).getFio());
                        if (res == true) {
                            qtyElem++;
                    ...
                    if (qtyElem == 0) {
                        ...
                        System.out.println("-------------------------------------");
                        System.out.println("Найдено " + qtyElem + " контакт(ов).");
                    }
                } catch (Exception ex) {
            ...
            //  5. Сортировка по ФИО
            if (provChoice == 5) {
                contacts.sort(Comparator.comparing(c -> c.fio));
                System.out.println("Сортировка по ФИО:" + contacts);
                continue;
            }
            // 6. Сортировка по дате рождения
            if (provChoice == 6) {
                contacts.sort(Comparator.comparing(c -> c.dateOfBirth));
                System.out.println("Сортировка по дате рождения:" + contacts);
                continue;
            }
